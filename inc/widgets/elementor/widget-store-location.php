...
class Widgets_CustomCategory extends \Elementor\Widget_Base {
	...
	public function __construct( $data = [], $args = null ) {
		...
		// wp_register_style( 'listivo-child-widgets', FWPWOOXSTORECHILD_BUILD_CSS_URI . '/widgets.css', [], apply_filters( 'futurewordpress/project/filesystem/filemtime', false, FWPWOOXSTORECHILD_BUILD_CSS_DIR_PATH . '/widgets.css' ) );
		...
		// wp_register_script( 'listivo-child-widgets', FWPWOOXSTORECHILD_BUILD_JS_URI . '/widgets.js', [ 'swiper' ], apply_filters( 'futurewordpress/project/filesystem/filemtime', false, FWPWOOXSTORECHILD_BUILD_JS_DIR_PATH . '/widgets.js' ), true );
	}
	/**
	 * Widget key.
	 * 
	...
	protected function render() {
		...
		$mapConfig = [];
		?>
		<div class="FWPWOOXSTORECHILDchildmap" data-map-config="<?php echo esc_attr( json_encode( $mapConfig ) ); ?>" data-map-reload="true"></div>
		<?php
	}
	/**
	...
	protected function _content_template() {
		...
		<# if ( settings.show_the_map == '1' ) { #>
			<div class="FWPWOOXSTORECHILDchildmap" data-map-config="{{{json_encode( $mapConfig )}}}" data-map-reload="true"></div>
		<# } #>
		<?php
	}
}
