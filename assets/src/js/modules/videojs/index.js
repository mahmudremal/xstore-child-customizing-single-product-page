 ...
 ( function ( $ ) {
   ...
   class FWPProject_VideoJS {
    constructor() {
      ...
      this.i18n        = {
        sureToSubmit: i18n?.sureToSubmit ?? 'Want to submit it? Canbe retaken.',
    ...
    afterInit() {
      const thisClass = this;var theInterval, playIcon;
      thisClass.initHooks();
      theInterval = setInterval( () => {
        playIcon = document.querySelector( '.vjs-record button.vjs-device-button.vjs-control.vjs-icon-av-perm:not([data-after])' );
        if( playIcon ) {
          playIcon.dataset.after = thisClass.i18n.click_here;
          playIcon.setAttribute( 'title', thisClass.i18n.click_here );
  ...
  //  `data:image/jpeg;base64,${file}`
    ...
    send( data ) {
      ...
      if( progress ) {progress.dataset.processing = true;progress.dataset.msg = thisClass.i18n.uploading + '...';}
      $.ajax({
        ...
        xhr: function() {
          ...
          xhr.upload.addEventListener("progress", function(evt) {
            if (evt.lengthComputable) {
              ...
              progress.dataset.msg = thisClass.i18n.uploading + ' (' + ( percentComplete * 100 ).toFixed( 0 ) + '%)...';
            }
          }, false);
